...
- **Dashboard Page (Analytics View):** A dedicated reporting interface built around a single visualizer rather than three fixed charts. Three controls — group by (`job_title` / `department` / `country`), aggregation (`min`, `max`, `avg`, `count`, `sum`), and a `created_at` date range — jointly determine one chart's contents, so any of the fifteen group-by/aggregation combinations is reachable from the same controls instead of needing a dedicated chart per dimension. `sum`/`count` render as a pie chart; `min`/`max`/`avg` render as a bar chart, since a single total or headcount reads naturally as a share of a whole, while a spread of values reads naturally as a comparison across categories.
