...
**Memory tuning (still in-process, not yet split out):** stacking Puma + the Solid Queue supervisor/dispatcher/worker as forked processes in one container — each carrying its own full eager-loaded Rails boot — exceeded Render's free-tier memory limit in practice. Until this is split into a separate worker service, `RAILS_MAX_THREADS` defaults to `2` (`config/puma.rb`, `config/database.yml`) and the Solid Queue worker runs `2` threads with a `100`-job dispatch batch size (`config/queue.yml`), down from `3`/`3`/`500` — a lower concurrency ceiling in exchange for a smaller baseline footprint, not a fix for the underlying trade-off above.
