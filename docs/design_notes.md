...
The five-stat aggregate bar above the list holds the min/max/avg/count/sum of the filtered set — a flex row on desktop (`md:flex`), a static `grid grid-cols-2` on mobile. The odd 5th card (Count) gets `col-span-2 mx-auto w-1/2` so it centers alone on its own row instead of sitting flush left; the earlier design used a horizontal scroll-snap strip here, dropped in favour of a wrapping grid so nothing is hidden off-screen. Amounts render through `aggregated_amount`, which shows an em dash until Step 6 normalizes a salary.

**Filtered to one currency, the bar switches basis from USD to that currency.** `normalized_usd_salary` is the aggregate basis everywhere by default — it's the only column that's comparable across a set of employees paid in different currencies. But once the `exchange_rate_id` filter narrows the listing to one specific rate, every employee left in scope shares that currency, so `local_salary` becomes directly comparable too, and reads as a more literal answer ("how much, in the currency they're actually paid in") than converting it back to USD would. `EmployeesController#filtered_currency` reuses `@exchange_rates` (already loaded for the filter dropdown) to resolve the code without a second query, and `Employee::SalaryAggregationService` takes the target `column:` explicitly rather than always assuming `normalized_usd_salary` — `local_salary` is the only other column it accepts, validated against an allowlist rather than interpolated freely, since it ends up directly inside a raw SQL fragment.
